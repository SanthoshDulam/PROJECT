...
def path_length_regulization_loss(generator, w, a):
    fake_images = generator(w)     # Generate the images from w
    random_images = torch.randn_like(fake_images)     # Get the corresponding random images
    output_var = (fake_images * random_images).sum()  # Output variation that we'd like to regularize
    cur_grad = grad(outputs=output_var, inputs=w)[0]  # Calculate the gradient with respect to the inputs    
    penalty = (((cur_grad - a) ** 2).sum()).sqrt()    # Calculate the distance from a
